<?php
if ( !class_exists( 'Ecome_MegaMenu' ) ) {
	class Ecome_MegaMenu
	{
		...
		function register_scripts( $hook )
		{
			...
			wp_localize_script( 'ecome-megamenu', 'ecome_ajax_backend', array(
					'ajaxurl'  => admin_url( 'admin-ajax.php' ),
					'security' => wp_create_nonce( 'ecome_ajax_backend' ),
				)
			);
		}
		...
		public function set_fonts()
		{
			$this->lits_fonts = array(
				array(
					'title' => esc_html__( 'Font Awesome', 'ecome' ),
					'fonts' => array(
						"flaticon-reload",
						"flaticon-layers",
						"flaticon-technology-1",
						"flaticon-print-button",
						"flaticon-mouse",
						"flaticon-console",
						"flaticon-smartphone",
						"flaticon-technology",
            ...
            <div class="icons-popup">
                <div class="inner">
					<?php if ( count( $this->lits_fonts ) > 0 ): ?>
                        <div class="ecome-modal-header ecome-text-center">
                            <label><?php echo esc_html__( 'Search Icon: ', 'ecome' ); ?></label>
                            <input type="text"
                                   placeholder="<?php echo esc_html__( 'Search a Icon...', 'ecome' ); ?>"
                                   class="ecome-icon-search"/>
                        </div>
		...
		public function addShortcodesCustomCss_megamenu()
		{
			...
			if ( $posts_array ) {
				...
				foreach ( $posts_array as $post ) {
					...
					$shortcodes_custom_css .= get_post_meta( $post->ID, '_Ecome_Shortcode_custom_css', true );
				}
				if ( !empty( $shortcodes_custom_css ) ) {
					wp_add_inline_style( 'ecome-style', $shortcodes_custom_css );
				}
			}
		}
	}
