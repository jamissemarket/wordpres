...
add_action( 'walker_nav_menu_custom_fields', 'ecome_add_custom_fields', 10, 4 );
function ecome_add_custom_fields( $item_id, $item, $depth, $args )
{
    ...
    <div class="container-megamenu container-<?php echo esc_attr( $depth ); ?>">
        <p class="item_icon_type">
            <label class="row_label"><strong><?php esc_html_e( 'Icon Settings', 'ecome' ); ?></strong></label><br/>
            <input <?php checked( 'none', $item_icon_type, true ); ?>
                    ...
                    for="none_item_icon_type_<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'None', 'ecome' ) ?></label>
            <input <?php checked( 'fonticon', $item_icon_type, true ); ?>
                    ...
                    for="fonticon_item_icon_type_<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Font icon', 'ecome' ) ?></label>
            <input <?php checked( 'image', $item_icon_type, true ); ?>
                    name="menu-item-megamenu-item_icon_type[<?php echo esc_attr( $item_id ); ?>]"
                    id="img_item_icon_type_<?php echo esc_attr( $item_id ); ?>" type="radio" value="image"><label
                    for="img_item_icon_type_<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e( 'Image', 'ecome' ) ?></label>
        </p>
        <p class="field-fonticon" <?php if ( $item_icon_type == 'fonticon' ): ?> style="display:block;"<?php endif; ?>>
            ...
            <a data-id="<?php echo esc_attr( $item_id ); ?>" class="button-select-icon"
               href="#"><?php esc_html_e( 'Select Icon', 'ecome' ); ?></a>
            ...
            <span class="image-field">
                ...
                <span class="image_icon" style="<?php if ( $preview ) {
                    ...
                    <img class="preview" src="<?php echo esc_url( $img_preview ); ?>"
                         alt="<?php esc_attr( 'Image preview' ); ?>"
                         title="<?php esc_attr( 'Image preview' ); ?>"/>
                </span>
                <a class="ecome_image_menu icon_image" data-item_id="<?php echo esc_attr( $item_id ); ?>"
                   href="#"><?php esc_html_e( 'Icon Image ', 'ecome' ); ?></a>
            </span>

        </p>
		<?php if ( 'megamenu' == $item->object ): ?>
            <p class="mega-menu-setting">
                <label class="row_label"><strong><?php esc_html_e( 'Mega menu settings', 'ecome' ); ?></strong></label>
                <br/>
                <label>Menu width
