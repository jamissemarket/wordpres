...
(function ($) {
    ...
    $(document).ready(function () {
        $('body').on('click', '.ecome_image_menu', function (e) {
            e.preventDefault();
            var t            = $(this),
            ...
            frame.on('select', function () {
                ...
                if ( t.hasClass('icon_image') ) {
                    var html           = '<img class="image-icon" src="' + attachment.url + '" alt="ecome" />';
        ...
        $(document).on('change keyup', '.ecome-icon-search', function () {

            var value  = $(this).val(),
                $icons = $(this).closest('.icons-popup').find('.font-item');

            $icons.each(function () {

                var $elem = $(this);

                if ( $elem.data('icon').search(new RegExp(value, 'i')) < 0 ) {
                    $elem.hide();
                } else {
                    $elem.show();
                }
